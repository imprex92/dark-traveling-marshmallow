    ...
    <>
      ...
      >
        ...
        <label className={styles.inputLabel} htmlFor="upload__input">
          <span
            className={`material-symbols-outlined text-white ${styles.inputImg}`}
          >
            photo_library
          </span>
      ...
      <div className={styles.filePreviews}>
        {validationObject &&
          validationObject.acceptedFiles.length > 0 &&
          validationObject.acceptedFiles.map((file, index) => (
            <div
              key={index}
              className={`${file.type.startsWith('image/') ? styles.previewItem_img : file.type.startsWith('video/') ? styles.previewItem_vid : styles.previewItem}`}
            >
              ...
              <button
                type="button"
                className={`${styles.removeBtn} btn waves-effect waves-light`}
                onClick={() => {
                  setFiles((prevFiles) =>
