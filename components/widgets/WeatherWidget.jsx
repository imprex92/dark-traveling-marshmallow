...
const WeatherWidget = () => {
  ...
  const { units, showWeatherWidget, latestWeather } = useSiteSettings(
    (state) => state.data
  );
  ...
  const fetchGeolocation = async (manual = false) => {
    ...
    if (!navigator.geolocation) {
      ...
      !manual &&
      latestWeather &&
      isSameHour(new Date(latestWeather.timestamp), new Date())
    ) {
  ...
  return (
    <>
      {isMaximized ? (
        <div
          className={`${styles.widgetWrapper} ${isMaximized ? '' : styles.minimized}`}
        >
          <div className={styles.widgetContainer}>
            ...
            {weatherObj ? (
              <img
                ...
                src={`${process.env.NEXT_PUBLIC_OPENWEATHER_ICON_URL}${weatherObj?.weather[0]?.icon}@2x.png`}
                alt="Weather icon"
              />
            ) : (
            ...
            <span
              ...
              className={`${locationLoading ? styles.reload_loading : styles.reload} material-icons`}
            >
              autorenew
            </span>
            <span
              onClick={() => toggleWidget(false)}
              className={`${styles.maximized} material-icons`}
            >
              chevron_right
            </span>
          </div>
        ...
        <span
          ...
          className={`${styles.minimized} material-icons`}
        >
          chevron_left
        </span>
      )}
