  ...
  return (
    ...
    >
      {popupToOpen !== null ? (
        <div
          onClick={(e) => {
            e.stopPropagation(), reset(e);
          }}
          className={`overlay ${popupToOpen ? 'open' : ''}`}
        >
          <Gallerypopup
            key={data.index}
            data={data}
      ...
      <div className="item-text">
        ...
        <span className="date">
          ...
          {pickedDateForPost?.seconds ? (
            <DateFormatter
              timeFromNow={false}
              timestamp={pickedDateForPost.seconds}
            />
          ) : null}
        </span>
        <div
          className={`${data?.index} dropdown-${data?.index}`}
          onClick={(e) => toggleDropdown({ event: e, trigger: 'dropdown' })}
        >
          Show more
          <i className={`${data?.index} material-icons chevron`}>
            chevron_left
          </i>
        </div>
        <div className={`hidden-section-${data?.index}`}>
          <span className="country">
            Country: {locationData?.countryOfPurchase}
          </span>
