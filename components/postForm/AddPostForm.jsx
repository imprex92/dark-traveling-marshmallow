...
import {
  ...
  getComponentValue,
  getGeoPointAndWeather,
} from 'components/utility/preparePostObject';
...
const AddPostForm = ({ dbUserData }) => {
  ...
  const { latestWeather } = useSiteSettings((state) => state.data) ?? {
    latestWeather: {},
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (Object.keys(postLocation).length > 0) {
      ...
      const { weatherData, geoPoint, error } = await getGeoPointAndWeather(
      ...
      postLocationData = {
        country:
          countryName ||
          getComponentValue(addressComponents, 'country') ||
          null,
        state:
          getComponentValue(addressComponents, 'administrative_area_level_1') ||
          postLocation?.locationData[0]?.vicinity ||
          null,
        city:
          getComponentValue(addressComponents, 'postal_town') ||
          getComponentValue(addressComponents, 'locality') ||
          postLocation?.locationData[0]?.name ||
          null,
        geopoint:
          geoPoint || postLocation?.coordinates || 'Coordinates not found',
    ...
    <>
      <div className={`${styles.stepTabs}`}>
        <div className={styles.navList}>
          <div
            onClick={(e) => {
              handleTabClick('content', e);
            }}
            className={`${activeTab === 'content' ? styles.tab_active : styles.tab}`}
          >
            <span className={`material-symbols-outlined ${styles.label}`}>
              line_start_circle
            </span>
        ...
        >
          <div className={`${styles.tabWrapper}`}>
            <div
              className={`${styles.tabContent} ${activeTab === 'content' ? styles.tabPanel_active : styles.tabPanel}`}
            >
              ...
              <div className={styles.fieldGroup}>
                ...
                >
                  <PlacesAutocomplete
                    ...
                    inputValue={(e) => (placesInputValue.current = e)}
              ...
              <div className={styles.fieldGroup}>
                <div className="input-field col s11 l6 datepicker-section">
                  ...
                  <input
                    id="datepicker"
                    type="text"
                    className={`datepicker ${styles.dateInput}`}
                  />
              ...
              <div className={styles.fieldGroup}>
                <div className="input-field col s6">
                  ...
                  <input
                    id="postTitle"
                    type="text"
                    className={`validate ${styles.titleInput}`}
                    placeholder=" "
              ...
              <div className={styles.fieldRow}>
                <div className="input-field col s12">
                  ...
                  <textarea
                    ...
                    id="postContent"
                    className={`materialize-textarea ${styles.postTextarea}`}
                    placeholder=" "
                  />
              ...
              <div className={`${styles.buttons}`}>
                <button
                  type="button"
                  className={`${styles.resetForm} btn waves-effect waves-light`}
                  onClick={clearForm}
                >
                  Clear
                </button>
            ...
            <div
              id="imgSectionTab"
              className={`${styles.tabContent} ${activeTab === 'images' ? `${styles.tabPanel_active} ${styles.imageTab_active}` : styles.tabPanel}`}
            >
              ...
              <Suspense
                fallback={
                  <CircularLoader loaderSize="big" loaderColor="default" />
                }
              >
                <FileInput
                  returnFiles={(f) => {
                    setFiles(f), handleTabClick('submit');
                  }}
                />
              </Suspense>
            </div>
            <div
              className={`${styles.tabContent} ${activeTab === 'submit' ? `${styles.tabPanel_active} ${styles.submitTab_active}` : styles.tabPanel}`}
            >
              ...
              >
                <PostSummary
                  formData={{
                    ...formData,
                    placesInputValue: placesInputValue.current,
                    files,
                    postLocation,
                  }}
                />
              ...
              <div className={styles.buttons}>
                ...
                {isSubmitting ? (
                  <CircularLoader
                    loaderSize="small"
                    loaderColor="spinner-red-only"
                    wrapperMargins="0 1rem 0.5rem 0"
                  />
                ) : null}
              </div>
            </div>
