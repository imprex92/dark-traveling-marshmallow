...
const SearchHistory = ({ fetchWeather, currentUser }) => {
  ...
  const clearHistory = async () =>
    await clearWeatherSearchHistory({ userID: currentUser.uid });
      ...
      {displayHistory.length === 0 ? (
        ...
        <>
          <span
            ...
            className={`${styles.clearHistory} material-symbols-outlined`}
          >
            delete_history
          </span>
          <div
            className={
              displayHistory.length < 6
                ? styles.historyItems
                : styles.historyItems_multi
            }
          >
            {displayHistory.map((city, index) => (
              ...
              >
                ...
                <span
                  onClick={() => removeHistoryItem(city)}
                  className={`${styles.removeItem} material-symbols-outlined`}
                >
                  close
                </span>
              </div>
