...
const SaveWeatherLocation = ({ currentWeather }) => {
  ...
  const [isWeatherChipsInitialized, setIsWeatherChipsInitialized] = useState(false);
  ...
  return weatherChips?.some(chip => chip.text === currentWeather?.data?.name) ? (
    ...
    <span onClick={() => addBookmark()} className={`${styles.bookmarkIcon} material-symbols-outlined`}>
      bookmark_add
    </span>
  )
}
