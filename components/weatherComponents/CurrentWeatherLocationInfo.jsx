    ...
    return (
        <div className="location-info">
            ...
            <div className='name-date'>
                <span className='name'>{error ? fallback : name}, {error ? fallback : sys?.country} <Flag fallback={ <Flag code="US" height="16" /> } code={sys?.country} height="16" /></span>
                <span className='date'>{error ? new Date().toLocaleDateString() : unixFormatter(dt)}</span>
            </div>
        </div>
    );
}
