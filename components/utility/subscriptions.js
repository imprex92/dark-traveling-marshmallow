...
function handleSaveNewPost({ userID, dataToSave, media }) {
  return new Promise(async (resolve, reject) => {
    ...
    const fileUploadPromises = media.map((file) => {
      const filePath =
        `userData/${userID}/${country}/${postDate}/${file.name}`.replace(
          /\s/g,
          '_'
        );
    ...
    try {
      ...
      await userDbRef.update({
        countriesVisited:
          projectFirebase.firestore.FieldValue.arrayUnion(country),
      });
...
const addWeatherChip = async ({ userID, payload, currentChips }) => {
  ...
  if (!payload.trim()) {
    return { data: null, error: 'Empty string' };
  }
  const userDbRef = projectFirestore
    .collection('testUserCollection')
    .doc(userID);
  ...
  try {
    const alreadyExists = currentChips.find(
      (chip) => chip.text.toLowerCase() === payload.toLowerCase()
    );
    const addChip = {
      id: uuidv4(),
      text: payload,
    ...
    if (alreadyExists) {
      return { data: null, error: 'Not saved. City already exists.' };
    }
...
const updateWeatherSearchHistory = async ({ userID, payload }) => {
  ...
  const userDbRef = projectFirestore
    .collection('testUserCollection')
    .doc(userID);
  const weatherDataRef = userDbRef
    .collection('weatherData')
    .doc('SearchHistory');
  ...
  try {
    ...
    await weatherDataRef.update({
      weatherSearchHistory:
        projectFirebase.firestore.FieldValue.arrayUnion(payload),
    });
...
const clearWeatherSearchHistory = async ({ userID }) => {
  ...
  try {
    ...
    const updateSearchHistory =
      useSiteSettings.getState().setWeatherSearchHistory;
    updateSearchHistory([]);
