...
const SkeletonWeather = ({ theme, position = null }) => {
  ...
  const MainSkeleton = () => {
    return (
      <>
        <SkeletonElement
          type={'title'}
          desktopCSS={{ marginTop: '2rem' }}
          globalCSS={{ width: '65%' }}
        />
        <SkeletonElement
          type={'text'}
          desktopCSS={{ margin: '4.5rem auto 1rem auto' }}
          globalCSS={{
            width: '50%',
            height: '20px',
        ...
        <SkeletonElement
          type={'text'}
          globalCSS={{ width: '40%', margin: '2rem auto' }}
        />
        <SkeletonElement
          type={'thumbnail'}
          globalCSS={{ margin: '2.5rem auto 2.5rem auto' }}
          mobileCSS={{ width: '150px' }}
          desktopCSS={{ width: '225px', height: '150px' }}
        />
        {!isMobile ? (
          <>
            <span
  ...
  return (
    <div className={`skeleton-wrapper weather-skeleton ${themeClass}`}>
      <div
        style={{ height: '100%' }}
        className={`skeleton-weather ${position}`}
      >
