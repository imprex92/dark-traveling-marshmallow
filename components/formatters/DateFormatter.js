    ...
    return(
        <>
            { timeFromNow ? 
                <span>
                    {
                        format(fromUnixTime(timestamp), 'MMMM do yyyy')
                    }, around  
                    {
...
export function ISODateFormatter({timestamp, timeFromNow = false}){
    ...
    return (
        <span>
            {formattedDate}
            {timeFromNow && `, around ${formatDistance(date, new Date(), { addSuffix: true })}`}
        </span>
    );
}
...
export function unixFormatter(timestamp) {
	return (
	  <>
		{
		  format(fromUnixTime(timestamp), 'eeee, dd/MMM/yyyy')
		}
	  </>
	);
  }
