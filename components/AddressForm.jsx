...
const AddressForm = (props) => {
  ...
  const handleSaveAddress = async (e) => {
    ...
    if (
      ...
      try {
        const address = await updateAddress(addressData, uid);
        if (address.status === 200) {
          M.toast({ text: `Status: ${address.status}, ${address.message}` });
          if (addressStatusMsg?.current) {
            ...
            setTimeout(() => {
              addressStatusMsg.current.style.display = 'none';
            }, 2000);
          }
          refs.address_1.current.value = addressData.addressLineOne;
          refs.address_2.current.value = addressData.addressLineTwo;
          refs.city.current.value = addressData.cityName;
          refs.zip.current.value = addressData.zipCode;
          refs.country.current.value = addressData.country;
        } else {
      ...
      <form className={`col s12 ${styles.addressForm}`}>
        <div className={`row ${styles.addressForm_row}`}>
          <div className="input-field col s12">
            ...
            />
            <label
              className={addressLineOne ? 'active' : ''}
        ...
        <div className={`row ${styles.addressForm_row}`}>
          <div className="input-field col s12">
            ...
            />
            <label
              className={addressLineTwo ? 'active' : ''}
        ...
        <div className={`row ${styles.addressForm_row}`}>
          <div className="input-field col s12 m6">
            ...
            <input
              ...
              className="validate"
            />
            <label className={cityName ? 'active' : ''} htmlFor="city">
              City
            </label>
          </div>
          <div className="input-field col s12 m6">
        ...
        <div className={`row ${styles.addressForm_row}`}>
          <div className="input-field col s12">
            ...
            <input
              ...
              className="validate"
            />
            <label className={country ? 'active' : ''} htmlFor="country">
              Country
            </label>
          </div>
        </div>
        ...
        <span ref={addressStatusMsg} className={styles.status_operation} />
      </form>
    </div>
  );
};
