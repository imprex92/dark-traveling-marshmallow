...
import { FILE_TYPE_IMAGES, PROFILE_IMG_MAX_SIZE } from 'components/utility/constants'

const settings = ({ userAuth, token, userAddress }) => {
	const { uid, name, picture, email, email_verified, phoneNumber = null, firebase } = token
	...
	const handleVerifyEmail = async () => {
		...
		try {
			...
			setLoaders((prevLoaders) => ({ ...prevLoaders, verificationEmail: false }));
		}
	};
	...
	const handleUpdateAccountInfo = async (e) => {
		...
		try {
			let update = await updateAccountData({
				...
				...(fileInput.current && fileInput.current.lastModified && { file: fileInput.current }),
				uid: uid
			});
				...
				<form className='col s12'>
					...
					<div style={{ gap: '1rem' }} className="row">
						<div className="input-field col s12 m6">
							<input placeholder=' ' autoComplete='name' ref={nameInput} type="text" id="full_name" defaultValue={name} className="validate" />
							<label className={nameInput.current ? 'active' : ''} htmlFor="full_name">Full name</label>
						</div>
						<div className="input-field col s12 m6">
							<input placeholder=' ' autoComplete='tel' disabled ref={phoneInput} type="text" id="tel" defaultValue={phoneNumber} className="validate" />
							<label className={phoneInput.current ? 'active' : ''} htmlFor="tel">Mobile number - Not yet Implemented</label>
						</div>
					</div>
					...
					<button disabled={loaders.profileStatus} onClick={(e) => handleUpdateAccountInfo(e)} className="btn-small waves-effect waves-light" >Save
						<i className="material-icons right">save</i>
					</button>
					<span ref={profileStatusMsg} className={styles.status_operation}></span>
					{loaders.profileStatus ? <CircularLoader loaderColor='default' loaderSize='small' /> : null}
				</form>
	...
	const AccountSettingsComp = () => {
		...
		return (
			<div className={styles.accountSettingsComp}>
						...
						? (
							<>
								<div style={{ position: 'relative' }} className={`row ${styles.fieldRow}`}>
									...
									{loaders.emailStatus ? <CircularLoader loaderColor='default' loaderSize='small' wrapperMargins='1.25rem 0 0 0.25rem' /> : null}
				...
				<button onClick={handleVerifyEmail} disabled={email_verified} className={`btn waves-effect waves-light btn-small ${email_verified && 'disabled'} ${styles.verifyEmailBtn}`} type="submit" name="action">
					{email_verified ? 'Already verified' : 'Verify email'}
					<i className="material-icons right">{email_verified ? 'done' : 'warning_amber'}</i>
				</button>
				...
				{loaders.passwordStatus || loaders.emailStatus ? <CircularLoader loaderColor='default' loaderSize='small' wrapperMargins='0.25rem 0 0 1rem' /> : null}
			</div>
		)
	}
		...
		<div className={styles.main}>
			...
			<div className={styles.settingsWrapper}>
				<aside className={styles.sidePanel}>
					<img loading='eager' style={{ objectFit: 'cover' }} className='circle' src={picture || "/assets/icons8-test-account.png"} width={120} height={120} />
					<div className={styles.sidePanelItems}>
						<div className={styles.userInfo}>
							<h4>{name ? name : 'No name yet.'}</h4>
							<span className={`${styles.tooltip} ${email_verified ? 'green-text' : 'red-text'}`}>
								{email}
								<span className={styles.tooltipText}>{email_verified ? 'Email verified.' : 'Not verified.'}</span>
							</span><br />
